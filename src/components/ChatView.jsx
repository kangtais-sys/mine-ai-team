...
const AGENT_DASHBOARDS = {
  chief: () => {
    ...
    return (
      <>
          ...
          {Object.entries(c).map(([name, val], i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderBottom: i < Object.keys(c).length - 1 ? '1px solid #1F1F1F' : 'none' }}>
                ...
                <span style={{ fontSize: 11, color: '#1D1D1F', flex: 1 }}>{name}</span>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F' }}>월간 비용 현황</span>
          ...
          {Object.values(costs).map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderBottom: i < Object.values(costs).length - 1 ? '1px solid #1F1F1F' : 'none' }}>
              ...
              {item.note && <span style={{ fontSize: 9, color: '#AEAEB2' }}>{item.note}</span>}
            </div>
          ))}
        </div>
        ...
        {/* Token usage */}
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F', marginBottom: 8 }}>Claude API 사용량</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#FFF' }}>{data?.claudeCalls?.today || 0}<span style={{ fontSize: 12, color: '#AEAEB2', fontWeight: 400 }}> calls today</span></div>
          <div style={{ fontSize: 10, color: '#AEAEB2', marginTop: 4 }}>예상 일비용: ~${((data?.claudeCalls?.today || 0) * 0.002).toFixed(3)}</div>
          <div style={{ fontSize: 9, color: '#D1D1D6', marginTop: 6 }}>절감 팁: 반복 댓글 캐싱, 배치 처리, 짧은 프롬프트</div>
        </div>

        {/* Daily report */}
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F', marginBottom: 8 }}>종합 일간 보고</div>
          {report ? (
            <>
              <div style={{ fontSize: 11, color: '#1D1D1F', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{report.report}</div>
              <div style={{ fontSize: 9, color: '#D1D1D6', marginTop: 6 }}>{report.date} · 매일 오전 8시 자동 생성</div>
            </>
          ) : (
    ...
    return (
      <>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          ...
          <div style={{ fontSize: 9, color: '#D1D1D6', marginTop: 6 }}>실시간</div>
        </div>
        ...
        {data?.recent?.length > 0 && (
          <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F', marginBottom: 8 }}>최근 발행</div>
            {data.recent.slice(0, 5).map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderBottom: i < Math.min(data.recent.length, 5) - 1 ? '1px solid #1F1F1F' : 'none' }}>
                ...
                <span style={{ fontSize: 11, color: '#1D1D1F', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</span>
                <span style={{ fontSize: 10, color: '#AEAEB2' }}>{p.createdAt?.slice(5, 10)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
            <div style={{ background: '#F5F5F7', borderRadius: 6, padding: '8px 10px', textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: '#6E6E73' }}>댓글 수신</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#FFF' }}>{logs.length}</div>
            </div>
            <div style={{ background: '#F5F5F7', borderRadius: 6, padding: '8px 10px', textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: '#6E6E73' }}>답글 완료</div>
          ...
          {/* Status messages */}
          {igLoading && !igLogs && (
            <div style={{ fontSize: 11, color: '#AEAEB2', textAlign: 'center', padding: 4 }}>로그 로딩 중...</div>
          )}
          {igError && (
            <div style={{ fontSize: 11, color: '#FF3B30', background: 'rgba(239,68,68,0.06)', borderRadius: 4, padding: '4px 8px', marginBottom: 8 }}>{igError}</div>
          )}
          {!igLogs && !igLoading && (
            <div style={{ fontSize: 10, color: '#AEAEB2', marginBottom: 4 }}>API 미연결 — 목업 데이터 표시 중</div>
          )}

          {/* Log Table */}
          <div>
            <div style={{ display: 'flex', gap: 6, fontSize: 10, color: '#AEAEB2', borderBottom: '1px solid #242424', paddingBottom: 4, marginBottom: 4 }}>
            ...
            {logs.slice(0, 5).map((l, i) => (
              <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '3px 0', borderBottom: i < Math.min(logs.length, 5) - 1 ? '1px solid #1F1F1F' : 'none' }}>
                <span style={{ width: 36, fontSize: 10, color: '#6E6E73', fontFamily: 'monospace' }}>{l.time}</span>
                ...
                <span style={{ flex: 1, fontSize: 10, color: '#6E6E73', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.reply}</span>
      ...
      <>
            ...
            <div key={si} style={{ marginBottom: si < 1 ? 14 : 0 }}>
              ...
              {section.items.map((item, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 10, color: '#AEAEB2' }}>{item.reviews}</span>
                </div>
              ))}
            </div>
          ))}
          <div style={{ fontSize: 9, color: '#D1D1D6', marginTop: 8 }}>수동 업데이트 · 마지막: {rankings?.updatedAt || '대기 중'}</div>
        </div>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14, marginTop: 12 }}>
          ...
          {[
            ...
            <div key={i} style={{ padding: '8px 0', borderBottom: i < 5 ? '1px solid #F5F5F7' : 'none' }}>
          ...
          <div style={{ fontSize: 9, color: '#D1D1D6', marginTop: 6 }}>별점 1-2점 / 환불·불만 키워드 → 관리 필요</div>
        </div>
  ...
  strategy: () => {
    ...
    <>
      ...
      <div style={{ marginTop: 12, background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F', marginBottom: 10 }}>진행 중 프로젝트</div>
        {[
          ...
          <div key={i} style={{ marginBottom: i < 4 ? 10 : 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ fontSize: 11, color: '#1D1D1F' }}>{p.name}</span>
              <span style={{ fontSize: 10, color: '#6E6E73' }}>{p.progress}%</span>
            </div>
            <div style={{ height: 3, background: '#F5F5F7', borderRadius: 2, overflow: 'hidden' }}>
      ...
      <div style={{ marginTop: 12, background: '#FFFFFF', border: '1px solid #E5E5EA', borderRadius: 8, padding: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#1D1D1F', marginBottom: 10 }}>경쟁사 분석 현황</div>
        {['롬앤', '클리오', '페리페라', '바닐라코', '마뗑킴'].map((c, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderBottom: i < 5 ? '1px solid #F5F5F7' : 'none' }}>
            <div style={{ width: 6, height: 6, borderRadius: 3, background: '#5E6AD2', flexShrink: 0 }} />
            <span style={{ fontSize: 11, color: '#1D1D1F', flex: 1 }}>{c}</span>
            <span style={{ fontSize: 10, color: '#34C759' }}>모니터링 중</span>
          </div>
        ))}
      </div>
    </>
