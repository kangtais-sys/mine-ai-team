...
const REVIEW_SHOT = localImg('/tmp/buck_real.png'); // 아마존 실제 후기 스크린샷 캡처(V. Michel Buck, 5★ Verified)
...
// ── 월요일 US 후기 후킹 카루셀 (표준순서 ①제목 ②후기 ③정보 ④CTA) ──
const MARKET = 'us';
// 가치형 "베이스 안 들뜨는 5가지" — 진짜 꿀팁이 주(主), 제품은 4번에 은근히 + 실제 후기 증거
const SLIDES = [
  // ① 가치 훅 (히어로 — 강한 후킹 + UGC 이미지)
  { type: 'cover',
    eyebrow: 'MAKEUP ARTIST SECRET',
    headline: 'Makeup\npilling\nby noon?',
    body: '5 fixes — and #1 isn’t your foundation.',
    image: PIN_COVER, credit: PIN },
  // ② Mistake 1 — 제품 은근히 + 실제 후기 증거 (앞당김)
  { type: 'reviewshot',
    headline: 'Mistake 1:\nthirsty skin.',
    body: 'Dry skin clings to makeup and shows texture. Prep with a **fast-absorbing mist** so foundation grips skin — real buyers notice:',
    image: REVIEW_SHOT,
    source: 'Amazon US · verified buyer · 4.8★ (29)',
    footnote: '(a 500-Dalton-size mist sinks in seconds — that’s the trick.)' },
  // ③ Mistake 2 (imageRight · 글래스 스킨)
  { type: 'info', variant: 'imageRight',
    headline: 'Mistake 2:\nyou rushed.',
    body: 'Foundation on tacky skin **slides and rolls**. Give your skincare a full **60 seconds** to sink in before base.',
    labels: ['let it absorb'],
    source: '02 / 05',
    image: PIN_DEWY, credit: PIN },
  // ④ Mistake 3 (imageLeft · 메이크업 베이스)
  { type: 'info', variant: 'imageLeft',
    headline: 'Mistake 3:\ntoo much.',
    body: 'Piling on product is the #1 cause. **Thin layers**, and **press** them in — never rub.',
    labels: ['less is more'],
    source: '03 / 05',
    image: PIN_MAKEUP, credit: PIN },
  // ⑤ Mistake 4 (textBig · 이미지 없음 = 텍스트 변주)
  { type: 'info', variant: 'textBig',
    num: '04',
    headline: 'Formula\nclash.',
    body: 'Water-based and silicone-based products can **ball up** together. Keep layers simple and let each one set.',
    labels: ['know your textures'],
    source: '04 / 05' },
  // ⑥ Mistake 5 (imageRight · 광채)
  { type: 'info', variant: 'imageRight',
    headline: 'Mistake 5:\nset it damp.',
    body: 'A dry, draggy sponge lifts everything off. **Dampen it, then press** — pulling is what starts the pilling.',
    labels: ['press, don’t drag'],
    source: '05 / 05',
    image: PIN_GLOW, credit: PIN },
  // ⑦ 참여 — 블랙: 댓글→DM 정보 + 추첨 선물 + 구독
  { type: 'engage',
    headline: 'Which one\nare you\nguilty of?',
    comment: 'Comment your number **1–5** and I’ll DM you the full prep guide.',
    giveaway: 'Every month, one commenter wins a full **500 Dalton series** set.',
    sub: 'Save it · send to a friend whose base always pills.',
...
for (let i = 0; i < SLIDES.length; i++) {
  ...
  const p = `${OUT}/monday_us_FINAL_0${i + 1}.png`;
